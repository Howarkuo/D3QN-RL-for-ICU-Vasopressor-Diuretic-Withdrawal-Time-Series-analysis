...
def create_clinical_table():
    ...
    # Raw data
    data = [
        ["n", " ", " ", "2533", "804", "1729", " "], # Replaced empty strings with space " "
    ...
    # Manually defined header to AVOID <B></B> error completely
    ...
    for row in data:
        ...
        for i, cell in enumerate(row):
            # 1. Escape special HTML characters
            formatted_cell = cell.replace("<", "&lt;")
            
            # 2. Safety check: Ensure no empty string enters the table
            if not formatted_cell or formatted_cell.strip() == "":
                formatted_cell = " "
    ...
    # Create node with 'plaintext' shape
    dot.node('struct1', label=html_label, shape='plaintext', fontname='Helvetica')
...
if __name__ == "__main__":
    try:
        ...
        dot.render('imputation_table', format='png', view=True)
        print("Success! Table generated.")
